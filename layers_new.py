    ...
    def __init__(self,
        ...
        if num_layers == 1:
            self.gat_layers.append(GATConv(
                ...
                feat_drop, attn_drop, negative_slope, last_residual, norm=last_norm, concat_out=concat_out, allow_zero_in_degree=True))
        else:
            # input projection (no residual)
            self.gat_layers.append(GATConv(
                in_dim, num_hidden, nhead,
                feat_drop, attn_drop, negative_slope, residual, create_activation(activation), norm=norm,
                concat_out=concat_out, allow_zero_in_degree=True))
            # hidden layers
...
if __name__ == '__main__':
    ...
    # model = GAT(in_dim=1433, num_hidden=128, out_dim=128, num_layers=2, nhead=4, nhead_out=4, activation='prelu',
    #             feat_drop=0.2, attn_drop=0.1, negative_slope=0.2, residual=False, norm=None, concat_out=True, encoding=True)
    model = DenseModel(in_dim=2708, num_hidden=128, out_dim=128, num_layers=2, dropout=0.5)
    feat = feat.unsqueeze(1).repeat(1, 5, 1)
    print(model)
